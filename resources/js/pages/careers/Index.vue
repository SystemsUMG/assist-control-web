        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="row">
                        <div class="col">
                            <h6>Tabla de Carreras</h6>
                        </div>
                        <div class="col text-end">
                            <button type="button" @click="OPEN('POST')" class="btn btn-dark btn-sm mb-3">Crear Carrera</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tr v-for="career in careers" :key="career.id">
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold ">{{ career.name }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold ">{{ career.description }}</span>
                                    </div>
                                </td>
                                <td class="align-middle">
                                    <a  class="text-success font-weight-bold text-xs cursor-pointer"  @click="OPEN('PUT', career.id)">Editar</a>
                                    &nbsp;
                                    <a class="text-danger font-weight-bold text-xs cursor-pointer" @click="OPEN('DELETE', career.id)">Eliminar</a>
                                </td>
                            </tr>
                            </tbody>
                            <tbody v-else>
...
export default {
    ...
    methods: {
        ...
        loadData(){
            ...
            setTimeout(
                function() {
                    axios({url: 'careers' , method: 'GET'})
                        .then((resp) => {
                            if(resp.data.records.length > 0) {
                                _this.show = true
                                _this.careers = resp.data.records
                                _this.icon = 'success'

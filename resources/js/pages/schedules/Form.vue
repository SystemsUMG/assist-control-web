            ...
            <div class="modal-content">
                <form @submit.prevent="SEND()">
                    ...
                    <div class="modal-body pt-0 px-4">
                        ...
                        <p class="text-uppercase text-sm">Información del Horario</p>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="begin_hour" class="form-control-label">Hora Inicio</label>
                                    <input id="begin_hour" class="form-control" type="time" min="06:00" max="22:00" v-model="data.begin_hour" :class="errors.begin_hour ? 'is-invalid' : ''">
                                    <small class="invalid-feedback">{{ errors.begin_hour ? errors.begin_hour[0] : '' }}</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="end_hour" class="form-control-label">Hora Fin</label>
                                    <input id="end_hour" class="form-control" type="time" min="06:00" max="22:00" v-model="data.end_hour" :class="errors.end_hour ? 'is-invalid' : ''">
                                    <small class="invalid-feedback">{{ errors.end_hour ? errors.end_hour[0] : '' }}</small>
                                </div>
                            </div>
                        </div>
                        <hr class="horizontal dark">
...
export default {
    ...
    computed: {
        OPEN: function() {
            let _this = this
            if(_this.method == 'PUT') {
                axios({url: '/schedules/' + _this.id, method: 'GET' })
                    .then((resp) => {
                        if (resp.data.result) {
    ...
    methods:{
        ...
        SEND: function(){
            ...
            if(_this.count == 1){
                ...
                this.errors = []
                setTimeout(function() {
                    axios({url: '/schedules' + method, method: 'POST', data: form })
                        .then((resp) => {
                            if(resp.data.result) {
                                _this.icon = 'success'

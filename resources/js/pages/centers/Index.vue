        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="row">
                        <div class="col">
                            <h6>Tabla de Centros</h6>
                        </div>
                        <div class="col text-end">
                            <button type="button" @click="OPEN('POST')" class="btn btn-dark btn-sm mb-3">Crear Centro</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tr v-for="center in centers" :key="center.id">
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold ">{{ center.name }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="px-3">
                                ...
                                <td class="align-middle">
                                    <a  class="text-success font-weight-bold text-xs cursor-pointer"  @click="OPEN('PUT', center.id)">Editar</a>
                                    &nbsp;
                                    <a class="text-danger font-weight-bold text-xs cursor-pointer" @click="OPEN('DELETE', center.id)">Eliminar</a>
                                </td>
                            </tr>
                            </tbody>
                            <tbody v-else>
...
export default {
    ...
    methods: {
        ...
        loadData(){
            ...
            setTimeout(
                function() {
                    axios({url: 'centers' , method: 'GET'})
                        .then((resp) => {
                            if(resp.data.records.length > 0) {
                                _this.show = true
                                _this.centers = resp.data.records
                                _this.icon = 'success'

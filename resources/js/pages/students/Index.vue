                ...
                <div class="card-header pb-0">
                    <div class="row">
                        ...
                        <div class="col text-end">
                            <button type="button" @click="OPEN('POST')" class="btn btn-dark btn-sm mb-3">Crear Estudiante</button>
                        </div>
                    </div>
                </div>
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            <thead>
                            <tr>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">No. de Carné</th>
                            ...
                            <tr v-for="student in students" :key="student.id">
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold">{{ student.carnet }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold">{{ student.name + ' ' + student.last_name }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="px-3">
                                ...
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold">{{ student.career_assigned.career.name }}</span>
                                    </div>
                                </td>
                                <td>
                                    <div class="px-3">
                                        <span class="text-secondary text-xs font-weight-bold">{{ student.career_assigned.center.name }}</span>
                                    </div>
                                </td>
                                <td class="align-middle">
                                    <a  class="text-success font-weight-bold text-xs cursor-pointer"  @click="OPEN('PUT', student.id)">Editar</a>
                                    &nbsp;
                                    <a class="text-danger font-weight-bold text-xs cursor-pointer" @click="OPEN('DELETE', student.id)">Eliminar</a>
                                </td>
                            </tr>
                            </tbody>
                            <tbody v-else>
...
export default {
    ...
    methods: {
        ...
        loadData(){
            ...
            setTimeout(
                function() {
                    axios({url: 'students' , method: 'GET'})
                        .then((resp) => {
                            if(resp.data.records.length > 0) {
                                _this.show = true
                                _this.students = resp.data.records
                                _this.icon = 'success'
